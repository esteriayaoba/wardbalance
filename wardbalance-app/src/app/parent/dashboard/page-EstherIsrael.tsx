...
export default function ParentDashboard() {
  ...
  useEffect(() => {
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      if (diff > 80 && !isLoading) { pulling = false; refetch(); }
    };
  ...
  if (error || !result) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center min-h-[300px]">
        ...
        <button onClick={() => refetch()} className="px-4 py-2 min-h-[44px] bg-primary text-white font-bold rounded-lg text-body-small hover:bg-primary-dark transition cursor-pointer">Try Again</button>
      </div>
    );
  }

  const outstandingNum = Number(result.totalOutstanding);
  ...
  return (
    <div className="space-y-6" aria-live="polite">
      ...
      <div className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm flex flex-col justify-between gap-4">
        <div>
          ...
          <div className={`text-display-small font-extrabold tabular-nums tracking-tight flex items-center gap-2 ${hasOutstanding ? "text-amber-600" : "text-green-600"}`}>
            ...
            {formatNaira(result.totalOutstanding)}
          </div>
        </div>
        <div className="flex items-center justify-between border-t border-neutral-100 pt-4">
          <span className="text-body-small text-neutral-500">
            {hasOutstanding
              ? `${result.wards.reduce((acc, w) => acc + w.invoiceCount, 0)} unpaid term invoices pending`
      ...
      <div className="space-y-3">
        <h3 className="text-label-medium text-neutral-900 font-bold uppercase tracking-wider">Student Registries ({result.wards.length})</h3>

        {result.wards.length === 0 ? (
            ...
            {result.recentPayments.map((p) => (
              <div key={p.id} className="flex justify-between items-start text-body-small">
                ...
                <div className="text-right">
                  ...
                  <span className={`text-[11px] px-1.5 py-0.5 rounded border uppercase font-extrabold mt-0.5 inline-block ${p.status === "recorded" ? "bg-green-50 text-green-600 border-green-100" : p.status === "void" ? "bg-red-50 text-red-600 border-red-100" : "bg-amber-50 text-amber-600 border-amber-100"}`}>
                    {p.status}
                  </span>
                </div>
              </div>
