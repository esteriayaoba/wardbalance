...
import { Loader2, ShieldAlert, History, Search, FileText, ArrowRight, XCircle } from "lucide-react";
...
export default function AuditLogsPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/audit")
      .then((r) => {
        if (r.status === 403) {
          throw new Error("Access Denied: Only the School Owner is authorized to view workspace audit logs.");
        }
        if (!r.ok) {
  ...
  if (error) {
    return (
      <div className="max-w-md mx-auto my-12 p-8 border border-red-200 bg-red-50/50 rounded-2xl text-center space-y-4 shadow-sm">
        <div className="w-12 h-12 bg-red-105 bg-red-100 text-error rounded-full flex items-center justify-center mx-auto">
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white p-4 rounded-xl border border-neutral-200 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="text-[11px] text-neutral-400 font-semibold uppercase tracking-wider">
          Displaying last {filteredLogs.length} events
        </div>
      </div>
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-neutral-200">
            {filteredLogs.map((log) => {
              ...
              return (
                <tr key={log.id} className="text-body-medium text-neutral-805 hover:bg-neutral-50/50">
                  ...
                  <td className="px-6 py-4 font-mono text-body-small text-neutral-500">
                    {log.ipAddress || "—"}
                  </td>
            ...
            {filteredLogs.length === 0 && (
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center text-neutral-400">
                  No system log history matching selected parameters.
                </td>
              </tr>
            )}
          </tbody>
      ...
      {/* Modal Dialog: Inspect Log JSON payload */}
      {selectedLog && (
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-4 bg-neutral-950 font-mono text-[12px] text-green-400 leading-normal">
              {selectedLog.previousValue && (
                <div className="space-y-1">
                  ...
                  <pre className="p-4 rounded bg-neutral-900 overflow-x-auto border border-neutral-800">
              ...
              {!selectedLog.previousValue && !selectedLog.newValue && (
                <div className="text-neutral-500 text-center py-6">
                  No state delta tracked for this event metadata.
                </div>
              )}
            </div>
