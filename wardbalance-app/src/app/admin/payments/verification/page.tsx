...
export default function VerificationQueuePage() {
  ...
  const loadSubmissions = async (status: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/payments/verification?status=${status}`);
  ...
  useEffect(() => {
    loadSubmissions(selectedStatus);
  }, [selectedStatus]);
  ...
  const handleAction = async (
    ...
    try {
      ...
      // Remove approved/rejected/reupload item from current view queue
      const updated = submissions.filter((s) => s.id !== activeSubmission.id);
    ...
    <div className="space-y-6 max-h-[calc(100vh-6rem)] flex flex-col h-full font-sans">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center p-8 text-center flex-1">
          ...
          <button
            onClick={() => loadSubmissions(selectedStatus)}
